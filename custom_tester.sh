...
test_from_file() {
	...
	do
		...
		if [[ $line == \#* ]] || [[ $line == "" ]] ; then
			...
			# INPUT=${INPUT%?}
		echo -n "$INPUT" | $MINISHELL_PATH/$EXECUTABLE 2>"tmp_err_minishell$i" >"tmp_out_minishell$i"
			exit_minishell=$?
			echo -n "enable -n .$NL$INPUT" | bash 2>"tmp_err_bash$i" >"tmp_out_bash$i"
			exit_bash=$?
			echo -ne "\033[1;34mSTD_OUT:\033[m "
			if ! diff -q "tmp_out_minishell$i" "tmp_out_bash$i" >/dev/null ;
			then
				echo -ne "❌  " | tr '\n' ' '
				((TEST_KO_OUT++))
				((FAILED++))
			...
			if [[ -s "tmp_err_minishell$i" && ! -s "tmp_err_bash$i" ]] || [[ ! -s "tmp_err_minishell$i" && -s "tmp_err_bash$i" ]] ;
			then
				echo -ne "❌  " |  tr '\n' ' '
				((TEST_KO_ERR++))
				((FAILED++))
